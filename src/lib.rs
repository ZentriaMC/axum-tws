    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MethodNotConnect => {
                write!(f, "http request method must be `CONNECT`")
            }
            Self::InvalidProtocolPseudoheader => {
                write!(f, "`:protocol` pseudo-header did not include 'websocket'")
            }
